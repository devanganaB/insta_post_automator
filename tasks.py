...
class MarketingAnalysisTasks:
	def product_analysis(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A detailed analysis report of the product website."
		)
	...
	def competitor_analysis(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A comparative analysis report of the product website and its top 3 competitors."
		)
	...
	def campaign_development(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="A detailed marketing campaign strategy and content ideas."
		)
	...
	def instagram_ad_copy(self, agent):
		return Task(description=dedent("""\
			...
			"""),
			agent=agent,
			expected_output="Three engaging and persuasive Instagram ad copy options."
		)
	...
	def take_photograph_task(self, agent, copy, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="Three detailed descriptions of potential photographs for the Instagram post."
		)
	...
	def review_photo(self, agent, product_website, product_details):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output="Three reviewed and approved options of photographs, each with a detailed description."
		)